...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("-----Finish-----")]
    [SerializeField] private float _finishDuration;
    ...
    private int _earnedCoinsOnLevel = 0;
    ...
    private void Awake()
    {
        ...
        forwardForce = 0;
        _earnedCoinsOnLevel = 0;      
    }

    private void Start() => CoinsData.IncreaseCoinsCount(0);
    ...
    private void Update()
    {
        ...
        var fov = boosted ? BoostFov : standartFov;

        mainCamera.fieldOfView = Mathf.MoveTowards(mainCamera.fieldOfView, fov, Time.deltaTime * fovChangeVelocity);

        var newFieldOfView = cameraFOVscaled ? 75 : standartFov;

        mainCamera.fieldOfView = Mathf.MoveTowards(mainCamera.fieldOfView, newFieldOfView, Time.deltaTime * 100);
    ...
    private void CoinEquip(GameObject other)
    {
        ...
        var part = Instantiate(coinsParticles, transform.position, Quaternion.identity);
        Destroy(part,1);
    ...
    private IEnumerator Finish(float duration)
    {
    ...
    private void IsFall()
    {
        ...
        {
            CoinsData.IncreaseCoinsCount(-_earnedCoinsOnLevel);
            SceneManager.LoadScene(SceneManager.GetActiveScene().name);
        }
    }
}